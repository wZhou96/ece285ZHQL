...
def post_processing(logits, image_size, gt_classes, anchors, conf_threshold,
    ...
    for boxes in predicted_boxes:
        ...
        selected_boxes.append(
            boxes[order][keep[:, None].expand_as(boxes)].view(-1,
                                                              6).contiguous())
...
def draw_boxes(img_id, predictions):
#     image = np.transpose(np.array(output_image.numpy()), (1, 2, 0))
#     image = (image + 1) / 2
#     image[image < 0] = 0
#     image[image > 1] = 1
#     norm_image = image * 255
#     norm_image.astype(int)
#     cv2.imwrite('tmp.png', norm_image)
#     img = cv2.imread('tmp.png')
#     R, G, B = cv2.split(img)
#     output_image = cv2.merge([B,G,R])
    img_pth = os.path.join('/datasets/ee285f-public/PascalVOC2012', "JPEGImages",
                                  "{}.jpg".format(img_id))
    output_image = cv2.imread(img_pth)
    height, width = output_image.shape[:2]
    width_ratio = float(448) / width
    height_ratio = float(448) / height
    ...
    if len(predictions) != 0:
        ...
        for pred in predictions:
            ...
            cv2.rectangle(output_image, (xmin, ymin), (xmax, ymax), 2)
            text_size = cv2.getTextSize(pred[5] + ' : %.2f' % pred[4], cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
            cv2.rectangle(output_image, (xmin, ymin), (xmin + text_size[0] + 3, ymin + text_size[1] + 4), -1)
            cv2.putText(
